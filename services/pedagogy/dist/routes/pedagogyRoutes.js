...
// ==========================================
// Create Exercise
router.post('/exercises', (0, auth_1.requireRole)(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
...
router.post('/evaluations', (0, auth_1.requireRole)(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
    const tenantId = req.tenantId;
    ...
    try {
        const evalId = crypto_1.default.randomUUID();
        const query = `
...
router.get('/evaluations/player/:id', async (req, res) => {
    ...
    try {
        if (user?.role === 'PARENT') {
            const link = await (0, db_1.executeTenantQuery)(tenantId, `SELECT 1 AS ok FROM parent_children WHERE parent_user_id = ? AND player_id = ? LIMIT 1`, [user.userId, playerId]);
            if (!link.length) {
                return res.status(403).json({ error: 'You can only view evaluations for your own children' });
            }
        }
        else if (user?.role === 'PLAYER') {
            const own = await (0, db_1.executeTenantQuery)(tenantId, `SELECT id FROM players WHERE user_id = ? LIMIT 1`, [user.userId]);
      ...
      LIMIT 10
    `;
        const history = await (0, db_1.executeTenantQuery)(tenantId, query, [playerId]);
        res.json(history);
    }
    catch (error) {
