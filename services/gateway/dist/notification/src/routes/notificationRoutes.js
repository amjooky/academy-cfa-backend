...
router.post('/dispatch', (0, auth_1.requireRole)(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
    ...
    try {
        const job = await notificationQueue_1.notificationQueue.add({
            tenantId,
            recipient,
            type,
...
router.get('/jobs/:id', async (req, res) => {
    try {
        const job = await notificationQueue_1.notificationQueue.getJob(req.params.id);
        if (!job) {
