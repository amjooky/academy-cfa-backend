import { callQwen, getQwenApiKey } from './qwen';
...
export function aggregateEvalMetrics(
  evaluations: Array<{ scores?: unknown; overall?: number; notes?: string }>
): { avgSpeed: number; avgTechnical: number; avgTactics: number; notes: string[] } {
  let speedSum = 0;
  let technicalSum = 0;
  let tacticsSum = 0;
  const notes: string[] = [];
  ...
  evaluations.forEach((e) => {
    ...
    if (e.notes?.trim()) notes.push(e.notes.trim());
  });
...
export function buildFallbackReport(ctx: PlayerReportContext): ProgressReportContent {
  const { avgSpeed, avgTechnical, avgTactics, playerName, position, team } = ctx;
  const strengths: string[] = [];
  const areasToImprove: string[] = [];
  const developmentPlanRecommendations: string[] = [];
  ...
  if (avgTechnical >= 7) {
    ...
    developmentPlanRecommendations.push(
      `Schedule ${team} small-sided rondo sessions (4v2) focused on one-touch passing and receiving on the back foot.`
    );
  }
  ...
  if (avgSpeed >= 7) {
    ...
    developmentPlanRecommendations.push(
      'Add twice-weekly 20m repeat-sprint blocks (6–8 reps, full recovery) before tactical segments.'
    );
  }
  ...
  if (avgTactics >= 7) {
    ...
    developmentPlanRecommendations.push(
      `Run ${team} phase-of-play drills on defensive shape (low block → mid-block transition) with coach freeze-and-reset cues.`
    );
  }
  ...
  if (developmentPlanRecommendations.length < 3) {
    developmentPlanRecommendations.push(
      `Tailor weekly homework for ${position} role: video review of last match clips plus 15-minute position-specific finishing or passing patterns.`
    );
  }

  if (developmentPlanRecommendations.length < 4 && ctx.evaluationNotes.length > 0) {
    developmentPlanRecommendations.push(
      `Address coach feedback: "${ctx.evaluationNotes[0].slice(0, 120)}${ctx.evaluationNotes[0].length > 120 ? '…' : ''}"`
    );
  }

  while (developmentPlanRecommendations.length < 3) {
    developmentPlanRecommendations.push(
      'Track weekly self-assessment scores (speed, technique, tactics) and review with coaching staff every 4 weeks.'
    );
  }

  const agePart = ctx.age != null ? `${ctx.age} years old` : 'age not on file';
  const summary =
    `${playerName} (${agePart}, ${position}, ${team}) shows a balanced profile with speed ${avgSpeed}/10, ` +
...
export async function generateProgressReport(
  ctx: PlayerReportContext
): Promise<{ report: ProgressReportContent; engine: string }> {
        ...
        [
          {
            role: 'system',
            content:
              'You are a professional youth football academy coach. Output valid JSON only, no markdown.',
          },
          { role: 'user', content: prompt },
        ],
        true
      );
      const parsed = JSON.parse(responseText) as ProgressReportContent;
      if (
        parsed.summary &&
