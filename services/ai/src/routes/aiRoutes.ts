...
import { callQwen, getQwenApiKey } from '../utils/qwen';
...
// ==========================================
router.post('/scout-report', requireRole(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
  ...
  try {
    ...
    const player = await executeTenantQuery(tenantId, playerQuery, [playerId]);
    ...
    const evaluationsQuery = `
      ...
      LIMIT 3;
    `;
    const evaluations = await executeTenantQuery(tenantId, evaluationsQuery, [playerId]);
...
router.post('/translate', async (req, res) => {
  ...
  try {
    ...
    if (apiKey) {
      ...
      const translatedText = await callQwen([
...
// ==========================================
router.post('/generate-plan', requireRole(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
...
// ==========================================
router.post('/progress-report', async (req: AuthenticatedRequest, res) => {
  const tenantId = req.tenantId!;
  const user = req.user!;
  const { playerId } = req.body;
  ...
  try {
    const allowed = await canAccessPlayer(tenantId, user.userId, user.role, playerId);
    if (!allowed) {
    ...
    const evaluations = await executeTenantQuery(
      tenantId,
      `SELECT scores, overall, notes, evaluated_at
       FROM evaluations WHERE player_id = ? ORDER BY evaluated_at DESC LIMIT 10`,
      [playerId]
    );

    const { avgSpeed, avgTechnical, avgTactics, notes } = aggregateEvalMetrics(evaluations);

    const upcomingEvents = await executeTenantQuery(
      tenantId,
      `SELECT e.title, e.type, e.starts_at, t.name AS team
       FROM events e
    ...
    const attendance = await executeTenantQuery(
      tenantId,
      `SELECT a.status, e.title, e.starts_at
       FROM attendance a
    ...
    const subs = await executeTenantQuery(
      tenantId,
      `SELECT status, plan, amount, currency, next_due FROM player_subscriptions WHERE player_id = ? LIMIT 1`,
      [playerId]
    );

    const recentEvents = upcomingEvents.map(
      (e: { title: string; type?: string; starts_at: Date | string; team?: string }) => {
    ...
    let attendanceSummary: string | null = null;
    ...
    const { report, engine } = await generateProgressReport(ctx);
