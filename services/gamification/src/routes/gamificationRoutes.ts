...
router.post('/xp/award', requireRole(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
  ...
  try {
    ...
    const playerRecord = await executeTenantQuery(tenantId, getQuery, [playerId]);
    ...
    const updateQuery = `
      ...
      WHERE id = ?
    `;
    await executeTenantQuery(tenantId, updateQuery, [newXpTotal, resolvedRank, playerId]);

    const selectQuery = `SELECT xp_total, rank, full_name FROM players WHERE id = ? LIMIT 1`;
    const selectResult = await executeTenantQuery(tenantId, selectQuery, [playerId]);
...
router.post('/badges/unlock', requireRole(['ACADEMY_ADMIN', 'COACH']), async (req, res) => {
  const tenantId = req.tenantId!;
  ...
  try {
    ...
    const existing = await executeTenantQuery(tenantId, checkQuery, [playerId, badgeName]);
    ...
    const query = `
      ...
      VALUES (?, ?, NOW())
    `;
    await executeTenantQuery(tenantId, query, [playerId, badgeName]);
...
router.get('/players/:id/achievements', async (req, res) => {
  ...
  try {
    ...
    const list = await executeTenantQuery(tenantId, query, [req.params.id]);
    res.json(list);
  } catch (error) {
...
router.get('/leaderboard', async (req, res) => {
  ...
  try {
    const query = `
      ...
      LIMIT 25
    `;
    const leaderboard = await executeTenantQuery(tenantId, query);
