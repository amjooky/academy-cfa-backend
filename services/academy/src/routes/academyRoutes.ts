...
router.post('/public/register', async (req, res) => {
  ...
  try {
    ...
    await executeTenantQuery(
      tenantId,
      `INSERT INTO users (id, email, password_hash, role, kyc_status, tenant_id, full_name, phone)
       VALUES (?, ?, ?, 'PARENT', 'pending', ?, ?, ?)`,
      [parentUserId, parentEmail, parentPassword, tenantId, parentName || null, parentPhone || null]
    );

    let childUserId: string | null = null;
    let childLoginEmail: string | null = null;
    ...
    if (createChildAccount) {
      ...
      const taken = await executeTenantQuery(
        tenantId,
      ...
      await executeTenantQuery(
        ...
        [childUserId, childLoginEmail, pwd, tenantId]
      );
    }
    ...
    await executeTenantQuery(
      tenantId,
      `INSERT INTO players (id, user_id, full_name, dob, position, status, photo_url, team)
       VALUES (?, ?, ?, ?, ?, 'pending', '', ?)`,
      [playerId, childUserId, fullName, dobDate, position, team || null]
    );
    await executeTenantQuery(
      tenantId,
      `INSERT IGNORE INTO parent_children (parent_user_id, player_id) VALUES (?, ?)`,
      [parentUserId, playerId]
    );

    if (team) {
      const teamRow = await executeTenantQuery(
        tenantId,
...
router.get('/profile', async (req, res) => {
  ...
  try {
    ...
    const profile = await executeTenantQuery(tenantId, query);
...
router.put('/profile', async (req, res) => {
  ...
  try {
    ...
    const check = await executeTenantQuery(tenantId, checkQuery);

    let query;
    let params;
    const profileId = check.length > 0 ? check[0].id : crypto.randomUUID();
...
router.post('/teams', async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?)
    `;
    await executeTenantQuery(tenantId, query, [teamId, name, ageGroup, coachId, season]);
    res.status(201).json({
...
router.get('/teams', async (req, res) => {
  ...
  try {
    ...
    const teams = await executeTenantQuery(tenantId, query);
    res.json(teams);
  } catch (error) {
...
router.post('/players', async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    await executeTenantQuery(tenantId, query, [playerId, userId, fullName, dob, position, photoUrl]);
    ...
    if (team) {
      ...
      const teamResult = await executeTenantQuery(tenantId, teamQuery, [team]);
      if (teamResult.length > 0) {
        ...
        const enrollQuery = `
          ...
          VALUES (?, ?)
        `;
        await executeTenantQuery(tenantId, enrollQuery, [teamId, playerId]);
      }
    }
...
router.get('/players', async (req, res) => {
  ...
  try {
    ...
    const players = await executeTenantQuery(tenantId, query);
    res.json(players);
  } catch (error) {
...
router.post('/teams/:id/players', async (req, res) => {
  ...
  try {
    const query = `
      ...
      VALUES (?, ?)
    `;
    await executeTenantQuery(tenantId, query, [teamId, playerId]);
    res.status(200).json({
...
router.get('/registrations', async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      ORDER BY p.full_name ASC
    `;
    const pending = await executeTenantQuery(tenantId, query);
    res.json(pending);
  } catch (error) {
...
router.post('/registrations/:id/approve', async (req, res) => {
  ...
  try {
    const updateQuery = `
      ...
      WHERE id = ?
    `;
    await executeTenantQuery(tenantId, updateQuery, ['active', playerId]);

    // Query updated player to return
    const getPlayerQuery = `SELECT * FROM players WHERE id = ? LIMIT 1`;
    const playerRows = await executeTenantQuery(tenantId, getPlayerQuery, [playerId]);
    ...
    if (teamName) {
      const teamQuery = `SELECT id FROM teams WHERE name = ? LIMIT 1`;
      const teamResult = await executeTenantQuery(tenantId, teamQuery, [teamName]);
      if (teamResult.length > 0) {
        const teamId = teamResult[0].id;
        const enrollQuery = `
          INSERT IGNORE INTO team_players (team_id, player_id)
    ...
    const parentLink = await executeTenantQuery(
      tenantId,
      `SELECT parent_user_id FROM parent_children WHERE player_id = ? LIMIT 1`,
      [playerId]
    );
    await executeTenantQuery(
      tenantId,
      `INSERT IGNORE INTO player_subscriptions (id, player_id, parent_user_id, plan, amount, currency, status, next_due)
       VALUES (?, ?, ?, 'monthly', 120, 'TND', 'pending', DATE_ADD(CURDATE(), INTERVAL 1 MONTH))`,
      [crypto.randomUUID(), playerId, parentLink[0]?.parent_user_id || null]
    );
...
router.post('/registrations/:id/reject', async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE id = ?
    `;
    await executeTenantQuery(tenantId, query, ['rejected', playerId]);
